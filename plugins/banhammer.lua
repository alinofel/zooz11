    ...
    local from_id = extra.from_id
     ...
     kick_user(member_id, chat_id)
      ...
      return
        ...
        send_large_msg(receiver, 'العضو @'..member..' ['..member_id..'] تم 🌚🚬حظره')
    ban_user(member_id, chat_id)
      elseif get_cmd == 'الغاء الحظر' then
        send_large_msg(receiver, 'العضو @'..member..' ['..member_id..'] نشال الحظر🌚🚬منه')
        local hash =  'banned:'..chat_id
        redis:srem(hash, member_id)
        return 'العضو '..user_id..' نشال الحظر🌚🚬منه'
      elseif get_cmd == 'حظر عام' then
        send_large_msg(receiver, 'العضو @'..member..' ['..member_id..'] تم حضره😂🚬عام خطية  ')
    banall_user(member_id)
      elseif get_cmd == 'الغاء العام' then
        send_large_msg(receiver, 'العضو @'..member..' ['..member_id..'] راح العام 😂🚬منه')
...
local support_id = msg.from.id
 if matches[1]:lower() == 'ايدي' and msg.to.type == "chat" or msg.to.type == "user" then
    if msg.to.type == "user" then
      return "🙈❤️ ايدي البوت : "..msg.to.id.. "\n\n🙈❤️ ايدي حسابك : "..msg.from.id.. "\n\n😍❤️ #المطور @ali_nofel"
    end
    ...
    elseif matches[1]:lower() == 'ايدي' then
      ...
      savelog(msg.to.id, name.." ["..msg.from.id.."] used /id ")
      return "ايدي المجموعه❤️" ..string.gsub(msg.to.print_name, "_", " ").. ":\n\n"..msg.to.id
    end
  end
...
if matches[1]:lower() == "قائمه المحظورين" then -- Ban list !
  ...
  if matches[1]:lower() == 'حظر' then-- /ban
    ...
    send_large_msg(receiver, 'العضو ['..matches[2]..'] تم 🌚🚬حظره')
      else
    local cbres_extra = {
    chat_id = msg.to.id,
    get_cmd = 'حظر',
    ...
    local chat_id = msg.to.id
      if string.match(matches[2], '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then
          ...
          return 'العضو ['..user_id..' ] راح العام 😂🚬منه'
    else
    local cbres_extra = {
      chat_id = msg.to.id,
      get_cmd = 'الغاء العام',
